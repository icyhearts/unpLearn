...
void server_run(int portNumber);
//void client_run();
void client_run(const char *ipaddr, int portNumber);
...
static void set_sockaddr(struct sockaddr_in *addr, int portNumber)
{
...
void server_run(int portNumber)
{
	...
	set_sockaddr(&srv_addr, portNumber);
		...
		for (i = 0; i < nfds; i++) {
			if (events[i].data.fd == listen_sock) {
				...
				epoll_ctl_add(epfd, conn_sock,
					      ...
					      EPOLLHUP);
			} else if (events[i].events & EPOLLIN) {// EPOLLIN = 0x1, normal input: events[i].event = 0x1
// for Ctl+D & Ctl+C in client: events = 0x2001, means both EPOLLIN and EPOLLRDHUP will be set
...
//	srv_addr->sin_addr.s_addr = INADDR_ANY;
	inet_pton(AF_INET, ipaddr, &srv_addr.sin_addr);
